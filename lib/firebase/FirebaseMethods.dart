...
import 'package:application_unknown/screens/verificationCode.dart';
...
class FirebaseMethods {
  ...
  sendInitialAttachMentMessage(
      String chatRoomId, String messageUid, Map initialInfoMap) async {
    return firestore
        .collection("userChats")
        .doc(chatRoomId)
        .collection("chats")
  ...
  sendFinalAttachMentMessage(
      String chatRoomId, String messageUid, Map finalInfoMap) async {
    return firestore
        .collection("userChats")
        .doc(chatRoomId)
        .collection("chats")
  ...
  deleteMessage(String messageUid, String chatRoomId,
      Map<String, dynamic> deleteMessageMap) async {
    await firestore
        .collection("userChats")
        .doc(chatRoomId)
        .collection("chats")
  ...
  Stream<DocumentSnapshot> getMsgSeenOrUnseen(
  ...
  updateTypingIndicator(
      String chatRoomId, Map<String, dynamic> typingIndicatorMap) async {
  ...
  Stream<DocumentSnapshot> getTypingIndicator(String chatRoomId) {
    return firestore.collection("userChats").doc(chatRoomId).snapshots();
  }
  ...
  Future<DocumentReference> createStatus(
      String userUid, Map statusInfoMap) async {
    return firestore
        .collection("status")
        .doc(userUid)
  ...
  updateLastStatus(String userUid, Map lastStatusMap) async {
    return firestore.collection("status").doc(userUid).set(lastStatusMap);
  }
  ...
  Stream<QuerySnapshot> statusUserCanSeewithNumber() {
    return firestore
        .collection("status")
        .where("canSee", arrayContains: "+1 650-555-3434")
        .snapshots();
  }
